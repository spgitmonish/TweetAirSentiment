...
# Using SVM and airline_sentiment as the class variable train the classifier
set.seed(7)
text_modelsvm <- svm(x = as.matrix(text_trainmatrix), 
                     y = as.factor(traindf$airline_sentiment),
                     kernel = "radial")

# Predict!
text_nbResults <- predict(text_modelnb, as.matrix(text_testmatrix))
text_svmResults <- predict(text_modelsvm, as.matrix(text_testmatrix))
...
reason_modelnb <- naiveBayes(as.matrix(reason_trainmatrix),
                             as.factor(traindf$airline_sentiment))

# Using SVM and airline_sentiment as the class variable train the classifier
reason_modelsvm <- svm(x = as.matrix(text_trainmatrix), 
                       y = as.factor(traindf$airline_sentiment),
                       kernel = "radial")
